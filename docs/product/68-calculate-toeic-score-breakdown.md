...
P6.7 - Calculate TOEIC Score Breakdown

## Purpose

Calculate score bands, raw counts, part accuracy, skill-tag breakdown, and time breakdown after practice tests.

## Detailed Scope

- Add ToeicScoreBreakdown domain/read model support.
...
Tables/read models must store session id, learner id, test type, content blueprint/version, status, started/submitted/expired timestamps, question assignments, answer state, and result id. `ToeicScoreBreakdown` is durable and survives restart.

## API Contract

Primary endpoint: `GET /api/learner/practice-tests/sessions/{sessionId}/result`. Related session endpoints must use `/api/learner/practice-tests/sessions/{sessionId}/...`. Errors include `TEST_CONTENT_UNAVAILABLE`, `TEST_SESSION_NOT_OWNED`, `TEST_SESSION_EXPIRED`, `TEST_ALREADY_SUBMITTED`.
...
1. Official-like scores must come from versioned conversion tables or be labelled as estimated bands.
...
rg -n "ToeicScoreBreakdown|practice-tests|TEST_SESSION" backend/src backend/tests docs/product
```
