...
P6.2 - Run TOEIC Part Tests

## Purpose

Run TOEIC-part-specific practice tests with part-correct question composition and result breakdowns.

## Detailed Scope

- Add PartTestSession domain/read model support.
...
Tables/read models must store session id, learner id, test type, content blueprint/version, status, started/submitted/expired timestamps, question assignments, answer state, and result id. `PartTestSession` is durable and survives restart.

## API Contract

Primary endpoint: `POST /api/learner/practice-tests/parts/{toeicPart}/start`. Related session endpoints must use `/api/learner/practice-tests/sessions/{sessionId}/...`. Errors include `TEST_CONTENT_UNAVAILABLE`, `TEST_SESSION_NOT_OWNED`, `TEST_SESSION_EXPIRED`, `TEST_ALREADY_SUBMITTED`.
...
1. Question counts and item types must follow the configured TOEIC part blueprint.
...
rg -n "PartTestSession|practice-tests|TEST_SESSION" backend/src backend/tests docs/product
```
