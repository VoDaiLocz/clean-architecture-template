...
P6.4 - Run TOEIC Reading Tests

## Purpose

Run a Reading section practice test across Parts 5-7 with passage-aware navigation and timing.

## Detailed Scope

- Add ReadingTestSession domain/read model support.
...
Tables/read models must store session id, learner id, test type, content blueprint/version, status, started/submitted/expired timestamps, question assignments, answer state, and result id. `ReadingTestSession` is durable and survives restart.

## API Contract

Primary endpoint: `POST /api/learner/practice-tests/reading/start`. Related session endpoints must use `/api/learner/practice-tests/sessions/{sessionId}/...`. Errors include `TEST_CONTENT_UNAVAILABLE`, `TEST_SESSION_NOT_OWNED`, `TEST_SESSION_EXPIRED`, `TEST_ALREADY_SUBMITTED`.
...
1. Reading section contains Parts 5-7 and preserves passage/question grouping.
...
rg -n "ReadingTestSession|practice-tests|TEST_SESSION" backend/src backend/tests docs/product
```

## Commit

`feat(p6.4): run TOEIC reading tests`
