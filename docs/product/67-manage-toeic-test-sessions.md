...
P6.6 - Manage TOEIC Test Sessions

## Purpose

Manage practice-test state, timer, resume, checkpoint, expiration, and final submission behavior.

## Detailed Scope

- Add ToeicTestSessionState domain/read model support.
...
Tables/read models must store session id, learner id, test type, content blueprint/version, status, started/submitted/expired timestamps, question assignments, answer state, and result id. `ToeicTestSessionState` is durable and survives restart.

## API Contract

Primary endpoint: `GET /api/learner/practice-tests/sessions/{sessionId}`. Related session endpoints must use `/api/learner/practice-tests/sessions/{sessionId}/...`. Errors include `TEST_CONTENT_UNAVAILABLE`, `TEST_SESSION_NOT_OWNED`, `TEST_SESSION_EXPIRED`, `TEST_ALREADY_SUBMITTED`.
...
1. State transitions and timer authority are backend-owned.
...
rg -n "ToeicTestSessionState|practice-tests|TEST_SESSION" backend/src backend/tests docs/product
```
