...
using Application.Features.Learner.ListeningTests;
...
public static class ListeningTestSessionTests
{
    ...
    public static void StartsSessionWithCorrectAssignedQuestions()
    {
        ...
        // Add 5 questions to parts 1,2,3,4
        for (int p = 1; p <= 4; p++)
        {
            for (int i = 0; i < 5; i++)
            {
        ...
        var handler = new StartListeningTestSessionHandler(repository);
        var command = new StartListeningTestCommand("learner1");
        ...
        if (result.AssignedQuestionIds.Count != 20) throw new Exception("Should have 20 questions (5 from 4 parts)");
        
        var diff = result.ExpiredAtUtc - DateTimeOffset.UtcNow.AddMinutes(45);
        if (Math.Abs(diff.TotalSeconds) > 5) throw new Exception("ExpiredAtUtc should be ~45 mins from now");

        var session = repository.GetListeningTestSession(result.SessionId);
        if (session == null) throw new Exception("Session not saved");
        if (session.Status != ListeningTestSessionStatus.Started) throw new Exception("Status should be Started");
        if (session.LearnerId != "learner1") throw new Exception("LearnerId mismatch");
        if (session.AssignedQuestionIds.Count != 20) throw new Exception("Assigned questions not saved");
    }
    ...
    public static void EnforcesExpiration()
    {
        ...
        repository.UpsertListeningTestSession(session);

        var submitHandler = new SubmitListeningTestSessionHandler(repository);
        var submitCmd = new SubmitListeningTestCommand("learner1", "s1", new Dictionary<string, string>());
        ...
        var loaded = repository.GetListeningTestSession("s1");
        if (loaded!.Status != ListeningTestSessionStatus.Expired) throw new Exception("Status should be Expired");
    }
    ...
    public static void PersistsAnswersAndFinalSubmit()
    {
        ...
        var submitCmd = new SubmitListeningTestCommand("learner1", "s1", answers);

        var submitResult = submitHandler.Handle(submitCmd, CancellationToken.None).GetAwaiter().GetResult();

        if (submitResult.Status != ListeningTestSessionStatus.Submitted) throw new Exception("Should be submitted");

        var loaded = repository.GetListeningTestSession("s1");
        if (loaded!.Status != ListeningTestSessionStatus.Submitted) throw new Exception("Status should be Submitted");
