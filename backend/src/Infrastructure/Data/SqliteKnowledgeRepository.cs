...
public sealed class SqliteKnowledgeRepository : IKnowledgeRepository, IDisposable
{
    ...
    public void Initialize()
    {
        ...
        command.CommandText =
            ...
            CREATE TABLE IF NOT EXISTS reading_test_sessions (
                session_id TEXT PRIMARY KEY,
                learner_id TEXT NOT NULL,
                status TEXT NOT NULL,
                started_at_utc TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS reading_test_session_questions (
                session_id TEXT NOT NULL,
                question_id TEXT NOT NULL,
                display_order INTEGER NOT NULL,
                PRIMARY KEY (session_id, question_id),
                FOREIGN KEY(session_id) REFERENCES reading_test_sessions(session_id)
            );

            CREATE TABLE IF NOT EXISTS reading_test_session_answers (
                session_id TEXT NOT NULL,
                question_id TEXT NOT NULL,
                answer TEXT NOT NULL,
                PRIMARY KEY (session_id, question_id),
            ...
            CREATE TABLE IF NOT EXISTS full_test_sessions (
                session_id TEXT PRIMARY KEY,
                learner_id TEXT NOT NULL,
                status TEXT NOT NULL,
                started_at_utc TEXT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS full_test_session_questions (
                session_id TEXT NOT NULL,
                question_id TEXT NOT NULL,
                display_order INTEGER NOT NULL,
                PRIMARY KEY (session_id, question_id),
                FOREIGN KEY(session_id) REFERENCES full_test_sessions(session_id)
            );

            CREATE TABLE IF NOT EXISTS full_test_session_answers (
                session_id TEXT NOT NULL,
                question_id TEXT NOT NULL,
                answer TEXT NOT NULL,
                PRIMARY KEY (session_id, question_id),
    ...
    public void UpsertReadingTestSession(ReadingTestSession session)
    {
        using var tx = connection.BeginTransaction();
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            command.CommandText =
                """
                INSERT INTO reading_test_sessions (
                    session_id,
                    learner_id,
                    status,
                    started_at_utc,
        ...
        {
            command.Transaction = tx;
            command.CommandText = "DELETE FROM reading_test_session_questions WHERE session_id = $session_id";
            command.Parameters.AddWithValue("$session_id", session.SessionId);
            command.ExecuteNonQuery();
        }
        ...
        for (int i = 0; i < session.AssignedQuestionIds.Count; i++)
        {
            ...
            command.CommandText =
                """
                INSERT INTO reading_test_session_questions (
                    session_id,
                    question_id,
                    display_order
                )
        ...
        {
            command.Transaction = tx;
            command.CommandText = "DELETE FROM reading_test_session_answers WHERE session_id = $session_id";
            command.Parameters.AddWithValue("$session_id", session.SessionId);
            command.ExecuteNonQuery();
        }
        ...
        foreach (var answer in session.Answers)
        {
            ...
            command.CommandText =
                """
                INSERT INTO reading_test_session_answers (
                    session_id,
                    question_id,
                    answer
                )
    ...
    public ReadingTestSession? GetReadingTestSession(string sessionId)
    {
        string learnerId;
        string statusStr;
        string startedAtStr;
        ...
        using (var qCommand = connection.CreateCommand())
        {
            qCommand.CommandText =
                """
                SELECT question_id
                FROM reading_test_session_questions
                WHERE session_id = $session_id
                ORDER BY display_order
                """;
            qCommand.Parameters.AddWithValue("$session_id", sessionId);
        ...
        using (var aCommand = connection.CreateCommand())
        {
            aCommand.CommandText =
                """
                SELECT question_id, answer
                FROM reading_test_session_answers
                WHERE session_id = $session_id
                """;
            aCommand.Parameters.AddWithValue("$session_id", sessionId);
            using var aReader = aCommand.ExecuteReader();
        ...
        return new ReadingTestSession
        {
            SessionId = sessionId,
            LearnerId = learnerId,
            Status = Enum.Parse<ReadingTestSessionStatus>(statusStr),
            StartedAtUtc = DateTimeOffset.Parse(startedAtStr),
            SubmittedAtUtc = submittedAtStr == null ? null : DateTimeOffset.Parse(submittedAtStr),
            ExpiredAtUtc = DateTimeOffset.Parse(expiredAtStr),
            AssignedQuestionIds = assignedQuestionIds,
    ...
    public void UpsertFullTestSession(FullToeicTestSession session)
    {
        using var tx = connection.BeginTransaction();
        ...
        using (var command = connection.CreateCommand())
        {
            ...
            command.CommandText =
                """
                INSERT INTO full_test_sessions (
                    session_id,
                    learner_id,
                    status,
                    started_at_utc,
        ...
        {
            command.Transaction = tx;
            command.CommandText = "DELETE FROM full_test_session_questions WHERE session_id = $session_id";
            command.Parameters.AddWithValue("$session_id", session.SessionId);
            command.ExecuteNonQuery();
        }
        ...
        for (int i = 0; i < session.AssignedQuestionIds.Count; i++)
        {
            ...
            command.CommandText =
                """
                INSERT INTO full_test_session_questions (
                    session_id,
                    question_id,
                    display_order
                )
        ...
        {
            command.Transaction = tx;
            command.CommandText = "DELETE FROM full_test_session_answers WHERE session_id = $session_id";
            command.Parameters.AddWithValue("$session_id", session.SessionId);
            command.ExecuteNonQuery();
        }
        ...
        foreach (var answer in session.Answers)
        {
            ...
            command.CommandText =
                """
                INSERT INTO full_test_session_answers (
                    session_id,
                    question_id,
                    answer
                )
    ...
    public FullToeicTestSession? GetFullTestSession(string sessionId)
    {
        string learnerId;
        string statusStr;
        string startedAtStr;
        ...
        using (var qCommand = connection.CreateCommand())
        {
            qCommand.CommandText =
                """
                SELECT question_id
                FROM full_test_session_questions
                WHERE session_id = $session_id
                ORDER BY display_order
                """;
            qCommand.Parameters.AddWithValue("$session_id", sessionId);
        ...
        using (var aCommand = connection.CreateCommand())
        {
            aCommand.CommandText =
                """
                SELECT question_id, answer
                FROM full_test_session_answers
                WHERE session_id = $session_id
                """;
            aCommand.Parameters.AddWithValue("$session_id", sessionId);
            using var aReader = aCommand.ExecuteReader();
        ...
        return new FullToeicTestSession(
            sessionId,
            learnerId,
            Enum.Parse<FullTestSessionStatus>(statusStr),
            DateTimeOffset.Parse(startedAtStr),
            submittedAtStr == null ? null : DateTimeOffset.Parse(submittedAtStr),
            DateTimeOffset.Parse(expiredAtStr),
            assignedQuestionIds,
            answers,
            resultId
        );
