...
public class ToeicPart6Engine : IToeicPartEngine
{
    public bool SupportsPart(int part) => part == 6;
    ...
    {
        if (question.ToeicPart != 6)
            throw new InvalidOperationException("This engine only supports TOEIC Part 6.");

        if (string.IsNullOrWhiteSpace(question.PassageId))
            throw new InvalidOperationException("Part 6 questions require a PassageId.");
    }
    ...
    private static string[] ParseChoices(string optionsJson)
    {
        try
        {
            ...
            {
                throw new InvalidOperationException("Part 6 questions must have exactly 4 choices.");
            }

            if (!dict.ContainsKey("A") || !dict.ContainsKey("B") || !dict.ContainsKey("C") || !dict.ContainsKey("D"))
            {
                throw new InvalidOperationException("Part 6 questions must have choices A, B, C, and D.");
            }
