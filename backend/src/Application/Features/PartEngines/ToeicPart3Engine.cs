...
public class ToeicPart3Engine : IToeicPartEngine
{
    public bool SupportsPart(int part) => part == 3;
    ...
    {
        if (question.ToeicPart != 3)
            throw new InvalidOperationException("This engine only supports TOEIC Part 3.");

        if (string.IsNullOrWhiteSpace(question.GroupId))
            throw new InvalidOperationException("Part 3 questions require a GroupId.");
    }
    ...
    private static string[] ParseChoices(string optionsJson)
    {
        try
        {
            ...
            {
                throw new InvalidOperationException("Part 3 questions must have exactly 4 choices.");
            }

            if (!dict.ContainsKey("A") || !dict.ContainsKey("B") || !dict.ContainsKey("C") || !dict.ContainsKey("D"))
            {
                throw new InvalidOperationException("Part 3 questions must have choices A, B, C, and D.");
            }
